...
knitr::opts_chunk$set(out.width='900px', dpi=200, echo = TRUE)
```
...
packagesneeded <- c("ggplot2", "dplyr", "RColorBrewer", "plotly")
...
pl[order(pl$LoanOriginationDate),]$CumulativePayments <- cumsum(pl[order(pl$LoanOriginationDate),"LP_CustomerPayments"])

pl$ListingCategory..numeric. <- factor(pl$ListingCategory..numeric.)

ListingCategory <- c('Not Available', 'Debt Consolidation', 'Home Improvement', 'Business',
...
pl$MainLoanStatus <- ifelse(pl$LoanStatus == "Completed" | pl$LoanStatus == "Current" |
                             pl$LoanStatus == "FinalPaymentInProgress" , "Paid/Current",
                     "Unpaid/Delayed")

pl$MeanCreditScore <- (pl$CreditScoreRangeUpper + pl$CreditScoreRangeLower)/2

#Necessary variables for correlation plot
variables <- c(pl$BorrowerAPR, pl$LenderYield, pl$EstimatedLoss, pl$ProsperRating..Alpha., pl$EmploymentStatus, pl$DebtToIncomeRatio, pl$StatedMonthlyIncome, pl$LoanOriginalAmount, pl$MainLoanStatus, pl$MeanCreditScore)

#Calculating actual returns
pl$ActualReturn <- (pl$LP_CustomerPayments - pl$LoanOriginalAmount) / pl$LoanOriginalAmount

#Good/Bad Returns
pl$Returns <- if_else(pl$ActualReturn>0, "Good","Bad")

#State Names
statenames <- c("Alabama", "Alaska", "Arizona", "Arkansas", "California", "Colorado", "Connecticut", "Delaware",
...
pl$LoanOriginationQuarter <- factor(pl$LoanOriginationQuarter, levels = c("Q4 2005","Q1 2006", "Q2 2006", "Q3 2006", "Q4 2006", "Q1 2007", "Q2 2007", "Q3 2007", "Q4 2007", "Q1 2008", "Q2 2008", "Q3 2008", "Q4 2008", "Q1 2009", "Q2 2009", "Q3 2009", "Q4 2009", "Q1 2010", "Q2 2010", "Q3 2010", "Q4 2010", "Q1 2011", "Q2 2011", "Q3 2011", "Q4 2011", "Q1 2012", "Q2 2012", "Q3 2012", "Q4 2012", "Q1 2013", "Q2 2013", "Q3 2013", "Q4 2013", "Q1 2014", "Q2 2014", "Q3 2014", "Q4 2014"))

#Re-ordering the factors on income range
pl$IncomeRange <- factor(pl$IncomeRange, levels = c("Not employed", "$0", "$1-24,999", "$25,000-49,999", "$50,000-74,999", "$75,000-99,999", "$100,000+", "Not displayed"))
...
```{r univariate plots, echo=FALSE, message=FALSE, warning=FALSE}
...
pl$ProsperRating..Alpha.<-factor(pl$ProsperRating..Alpha., levels=c("AA", "A", "B", "C", "D", "E", "HR")) 
...
ggplot(aes(ProsperRating..Alpha.,MeanCreditScore), data = subset(pl,ProsperRating..Alpha.!="")) + 
  geom_boxplot() +
  labs (x = "Prosper Rating",y = "Mean Credit Score") +
  theme(text = element_text(size=8.5))
```
...
ggplot(aes(ProsperRating..Alpha.,LenderYield*100), data = subset(pl,ProsperRating..Alpha.!="")) + 
  geom_boxplot() +
...
ggplot(aes(DebtToIncomeRatio,BorrowerAPR), data = pl) + 
  geom_point(alpha = 0.5) +
  scale_x_sqrt() +
  labs (x = "Debt to Income Ratio",y = "Borrower APR") +
  theme(text = element_text(size=8.5))
```
...
ggplot(aes(CurrentDelinquencies, fill = ProsperRating..Alpha.), data = pl) + 
  geom_density(alpha = 0.75) + 
  facet_wrap(~ProsperRating..Alpha., scales = "free", ncol = 2) +
  scale_fill_brewer(palette="Set1") +
  labs (x = "Current Delinquencies",y = "Prosper Rating") +
  theme(text = element_text(size=8.5)) +
...
ggplot(aes(CurrentDelinquencies,BorrowerAPR*100, color = ProsperRating..Alpha.), data = pl) + 
...
ggplot(aes(AmountDelinquent,BorrowerAPR*100, color = ProsperRating..Alpha.), data = pl) + 
  geom_point(alpha = 0.8) +
  scale_x_sqrt() +
  scale_colour_brewer(palette="Set2") +
...
```{r, echo=FALSE, message=TRUE, warning=FALSE}
ggplot(aes(EstimatedReturn,LoanOriginalAmount, color = ProsperRating..Alpha.), data = pl) + 
  geom_point(alpha = 0.8) +
...
The estimated return seems to be more dependant on the prosper rating than the original loan amount.
...
ggplot(aes(x=LoanOriginationDate), data = completedloans) + 
  ...
  geom_line(aes(y = CumulativePayments/1000000, 
                color = if_else(CumulativePayments>=CumulativeLoanAmount, 
                                "Good Customer Payments", "Poor Customer Payments"))) + 
  scale_color_manual(values=c("green", "blue", "red")) +
  #facet_wrap(~IncomeRange) +
  labs (x = "Loan Origination Date",y = "Total Loan amount and Payments in Millions") +
  theme( text = element_text(size=8.5)) +
...
```{r, echo=TRUE, message=TRUE, warning=FALSE}
filter(pl,(pl$LoanStatus == "Completed" & pl$CumulativePayments < pl$CumulativeLoanAmount & pl$Returns == "Bad")) %>%
arrange(LP_CustomerPayments) %>% select(LoanOriginationDate, LoanOriginalAmount, LP_CustomerPayments) %>% head
```
...
```{r, echo=FALSE, message=TRUE, warning=FALSE}
group_by(pl[pl$DebtToIncomeRatio!="",], ListingCreationYear, MainLoanStatus) %>%
  summarise(., avg_debt = round(mean(DebtToIncomeRatio, na.rm = T),1)) %>%

ggplot(aes(ListingCreationYear, avg_debt, fill = MainLoanStatus)) + 
  geom_bar(stat = "identity", position = "dodge") + 
  scale_fill_brewer(palette = "Set2") +
  labs (x = "Listing Creation Year",y = "Average Debt to Income Ratio") +
  #coord_flip() +
  theme(text = element_text(size=8.5)) 
```

The debt to income ratio of the borrowers has spiked during 2006-2008, possibly due to the economic deprssion in the country or non-existant filtering of the borrowers. Ut then stabilized at an average of 0.3 from the year 2012.

```{r, echo=FALSE, message=TRUE, warning=FALSE}
ggplot(aes(EstimatedReturn, LoanOriginalAmount, color = Returns), data = pl) + 
...
```{r, echo=FALSE, message=TRUE, warning=FALSE}
ggplot(aes(ActualReturn, LoanOriginalAmount, color = Returns), data = subset(pl,LoanStatus == "Completed")) + 
  stat_summary(fun.y = "mean", geom = "bar") +
  facet_wrap(~ProsperRating..Alpha., scales ="free") +
...
ggplot(aes(ActualReturn, EstimatedReturn, color = Returns), data = subset(pl,LoanStatus == "Completed")) + 
...
```{r, echo=FALSE, message=TRUE, warning=FALSE}
sumStates <- group_by(pl[nchar(as.character(pl$BorrowerState)) > 0,], BorrowerState) %>%
  summarise(., total_issued = round(sum(LoanOriginalAmount/1e6),1), 
            ...
            avg_borrowerapr = round(mean(BorrowerAPR),1))

#sumStates$map_amnt = with(sumStates, pmin(total_issued, 500))

sumStates$hover <- with(sumStates, paste(state.name[match(BorrowerState, state.abb)], '<br>', 
                                         "Total amount issued:", prettyNum(total_issued, big.mark = ","), 'Million USD <br>',
                                         "Number of Loans:", prettyNum(count, big.mark = ","), '<br>',
                                         "Average Debt to Income Ratio:", prettyNum(avg_debt, big.mark = ","), '<br>',
                                         "Average State Income:", prettyNum(avg_state_income, big.mark = ","), '<br>',
                                         "Average Borrower APR:", prettyNum(avg_borrowerapr, big.mark = ",")))

g = list(scope = 'usa', projection = list(type = 'albers usa'), showlakes = T, lakecolor = toRGB('white'))
...
```{r, echo=FALSE, message=TRUE, warning=FALSE}
ggplot(aes(BorrowerAPR*100,MeanCreditScore, color = Term), data = subset(pl,MeanCreditScore>100)) + 
  geom_point(alpha = 0.5) +
  stat_smooth(method = 'lm', formula = y ~ poly(x,2), aes(colour = Term), se = FALSE) +
...
```{r, echo=FALSE, message=TRUE, warning=FALSE}
ggplot(aes(DebtToIncomeRatio,LenderYield, color = ProsperRating..Alpha.), data = subset(pl,ProsperRating..Alpha.!= "")) + 
  geom_point(alpha = 0.8) +
  scale_x_continuous(limits = c(0,1)) +
  scale_y_log10(breaks = c(0.1,0.2,0.3)) +
...
The Prosper loan data set has 113,937 loan listings and  81 variables from Nov 2005 - March 2014. I was able to analyze around 20 of them and also created a few additional variables to help in the analysis. The data is rich with information on borrower, credit, debt, investor, loan and all of their geographic data. 
...
Although, there were too many variables and more than 100k records available, considering this to be 9 years worth of data for US wide loans, it is a very small data set and having additional data could highlight some patterns and relations that are not strongly visible now. 
