...
variables <- c(pl$BorrowerAPR, pl$LenderYield, pl$EstimatedLoss, 
               pl$ProsperRating..Alpha., pl$EmploymentStatus, 
               pl$DebtToIncomeRatio, pl$StatedMonthlyIncome, 
               pl$LoanOriginalAmount, pl$MainLoanStatus, pl$MeanCreditScore)
...
pl$IncomeRange <- factor(pl$IncomeRange, levels = c("Not employed", "$0", 
...
```{r, echo=FALSE, message=TRUE, warning=FALSE}
ggplot(aes(ActualReturn, LoanOriginalAmount, color = Returns), 
...
```{r, echo=FALSE, message=TRUE, warning=FALSE}
ggplot(aes(DebtToIncomeRatio,LenderYield, color = ProsperRating..Alpha.), 
       data = subset(pl,ProsperRating..Alpha.!= "")) + 
  geom_point(alpha = 0.8) +
  scale_x_continuous(limits = c(0,1)) +
